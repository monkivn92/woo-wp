					...
					if ( isset( $field['required'] ) && $field['required'] && ( ! isset( $this->posted[ $key ] ) || "" === $this->posted[ $key ] ) ) {
						switch ( $fieldset_key ) {
							...
							case 'billing' :
								...
								//$field_label = 'Xin quý khách vui lòng điền đầy đủ thông tin ở mục "Thông tin thanh toán". Xin cảm ơn!';
							break;
							default :
								$field_label = $field['label'];
							break;
						}
						wc_add_notice( apply_filters( 'woocommerce_checkout_required_field_notice', sprintf( _x( 'Xin vui lòng điền thông tin vào mục %s.', 'Xin vui lòng điền thông tin vào mục FIELDNAME.', 'woocommerce' ), '<strong>' . $field_label . '</strong>' ), $field_label ), 'error' );
					}
